<div class="relative mt-3 md:mt-0" x-data="{ isOpen: true }" @click.away="isOpen = false" >
    <input 
        ...
        @focus="isOpen = true" 
        @keydown"isOpen = true" 
        @kedyown.shift.tab="isOpen = false"                     
        @keydown.escape.window="isOpen = false"
        >
     <div class="absolute top-0"> {{-- search ikona  --}}
         <a href="">
     ...
     <div wire:loading class="spinner top-0 right-0 mr-4 mt-3"></div>
      ...
      @if(strlen($search)>=2)
         <div class="z-50 absolute bg-gray-800 text-sm rounded w-64 mt-4" 
            x-show.tranition.opacity="isOpen"
           
        >
            @if($searchResults->count() > 0) 
          <ul>
              @foreach($searchResults as $result)
                <li class="boreder-b boreder-gray-700"> 
                    <a
                         href="{{ route('movies.show', $result['id']) }}" 
                         class="block hover:bg-gray-700 flex items-center px-3 py-3 ease-in-out duration-150"
